...
string CommonUtil::GetFileVersion(string filePath) {
...
BOOL CommonUtil::FileExist(std::string file) {
	struct stat buffer;
	return (stat(file.c_str(), &buffer) == 0);
}

wstring CommonUtil::itow(int i) {
	wstring data;
	wchar_t t[10];
	wmemset(t, 0, 10);
	wsprintf(t, TEXT("%d"), i);
...
string CommonUtil::w2s(wstring wstr) {
	string result;
...
string CommonUtil::wstring2string(wstring wstr) {
	return w2s(wstr);
}

wstring CommonUtil::s2w(string str) {
	wstring result;
...
wstring CommonUtil::string2wstring(string str) {
	return s2w(str);
}

const char* CommonUtil::u2g(string str) {
...
string& CommonUtil::Replace(string& str, const string& old_value, const string& new_value) {
...
string CommonUtil::Base64_Encode(const char* data, int len) {
...
string CommonUtil::Base64_Encode(string str) {
	return Base64_Encode(str.c_str(), str.size());
}

string CommonUtil::Base64_Decode(const char* data, int len, int& olen) {
...
string CommonUtil::Base64_Decode(string data, int& olen) {
	return Base64_Decode(data.c_str(), data.size(), olen);
}

std::wstring CommonUtil::RegReadView(HKEY root, LPCWSTR path, LPCWSTR key, DWORD type) {
	wstring value(TEXT(""));
	HKEY hKey;
...
DWORD CommonUtil::ReadInt(DWORD address) {
	DWORD intValue = 0;
	intValue = *(DWORD*)address;
	return intValue;
}

string CommonUtil::ReadAsciiString(DWORD address) {
	string sValue = "";
...
wstring CommonUtil::ReadUnicodeString(DWORD address) {
	wstring value;
