...
void ConsoleUtil::OpenConsole() {
	if (!mConsoleOpen) {
		if (AllocConsole()) {
			...
			char nt[100];
			sprintf_s(nt, "日志[%s]-%lld-%d",remoteProgramName, GetTickCount64(), GetCurrentProcessId());
			SetConsoleTitleA(nt);
			Sleep(100);
				...
				if (menu) {
					if (!needMenu) {
						if (RemoveMenu(menu, 0xF060, 0)) {
							sprintf_s(nt, "日志[%s]-%lld-%d-已屏蔽关闭按钮", remoteProgramName, GetTickCount64(), GetCurrentProcessId());
							SetConsoleTitleA(nt);
						}
					}
				}
